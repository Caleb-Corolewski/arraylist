 ...
 * get you started.
 * 
 * @author mvail, mhthomas, awinters ccorolewski
 */
...
//			break;
	...
	/** Scenario: [A,B,C] -> iteratorRemoveAfterNextA -> [BC] 
	 * @return [BC] after iteratorRemoveAfterNextA
	 */
	...
	/** Scenario: [A,B,C] -> iteratorRemoveAfterNextNextB -> [AC] 
	 * @return [AC] after iteratorRemoveAfterNextNextB
	 */
	...
	/** Scenario: [A,B,C] -> iteratorRemoveAfterNextNextNextC -> [AB] 
	 * @return [AB] after iteratorRemoveAfterNextNextNextC
	 */
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_iterNextNextRemove_testIterNext", testIterNext(iterNextAfterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 1))), null,Result.NoSuchElement));
	...
	private void testThreeElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_iterNextNextRemove_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(scenarioName + "_iterNextRemoveNext_testIterHasNext", testIterHasNext(iterNextAfterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 1))), Result.True));
	...
	/**
	 * helper for testing iterator when next is called after a remove is called
	 * @param iterator
	 * @return the same iterator after a call to next
	 */
